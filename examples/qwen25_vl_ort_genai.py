...
Usage::
    ...
    python examples/qwen25_vl_ort_genai.py

    # With an image:
    python examples/qwen25_vl_ort_genai.py --image testdata/pipeline-cat-chonk.jpeg

    # Compare ORT GenAI output with HuggingFace transformers:
    python examples/qwen25_vl_ort_genai.py --image testdata/pipeline-cat-chonk.jpeg --compare-hf

    # Use a pre-built model directory:
    python examples/qwen25_vl_ort_genai.py --model-dir output/qwen25vl/

    # Build and save (skip inference):
    python examples/qwen25_vl_ort_genai.py --save-to output/qwen25vl/
"""
...
def _write_genai_config(config, output_dir: str) -> None:
    """Write genai_config.json for the Qwen2.5-VL 3-model split."""
    genai_config = {
        "model": {
            "bos_token_id": 151643,
            "context_length": 4096,
...
def _update_resize_for_image(
    ...
    # Round to nearest multiple of factor (same as HF smart_resize)
...
def generate_with_image(
    model_dir: str, prompt: str, image_path: str, max_new_tokens: int
) -> str:
    """Run multimodal generation (text + image) with onnxruntime-genai.

    Uses the ORT GenAI multimodal processor to preprocess image and text
    into the inputs expected by the 3-model pipeline.  The vision ONNX
    model computes all packed-attention values internally from
    ``image_grid_thw``.
    """
    ...
    hf_processor = HFProcessor.from_pretrained(MODEL_ID)
    messages = [
        {
            "role": "user",
            "content": [
    ...
    full_prompt = hf_processor.apply_chat_template(
        messages, tokenize=False, add_generation_prompt=True
    )

    # Use ORT GenAI processor for image + text
...
def main():
    parser = argparse.ArgumentParser(
        description="Qwen2.5-VL generation with onnxruntime-genai.",
    )
    parser.add_argument(
        "--model-id",
        default=MODEL_ID,
    ...
    parser.add_argument(
        "--image",
        default=None,
        help="Path to an image file for multimodal generation.",
    )
    parser.add_argument(
        "--prompt",
        default=None,
        help="Text prompt. Defaults depend on whether --image is provided.",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
    ...
    if args.model_dir:
        ...
        model_dir = os.path.join("output", "qwen25_vl")
