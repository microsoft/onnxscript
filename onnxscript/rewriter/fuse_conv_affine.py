...
from ._ir_utils import get_singleton_value, get_const_value
...
class _ConvAffineFusionBase(pattern.RewriteRuleClassBase):
    def check(self, context, x: ir.Value, w: ir.Value, b: ir.Value, scale: ir.Value, offset: ir.Value, conv_out: ir.Value) -> MatchResult:
...
class AffineConvFusion(_ConvAffineFusionBase):
    ...
    def pattern(self, op, x: ir.Value, w: ir.Value, b: ir.Value, scale: ir.Value, offset: ir.Value) -> ir.Value:
        return op.Conv(x * scale + offset, w, b, pads=[0, 0, 0, 0], _allow_other_attributes=True, _outputs=['conv_out'])
    def rewrite(self, op: ir.tape.Tape, x: ir.Value, w: ir.Value, b: ir.Value, scale: ir.Value, offset: ir.Value, conv_out: ir.Value) -> ir.Value:
        ...
        offset_bias = ir.tensor(b_value + np.sum(w_value * offset_value, axis=(1, 2, 3), keepdims=False))
...
class ConvAffineFusion(_ConvAffineFusionBase):
    ...
    def pattern(self, op, x: ir.Value, w: ir.Value, b: ir.Value, scale: ir.Value, offset: ir.Value) -> ir.Value:
        return op.Conv(x, w, b, _allow_other_attributes=True, _outputs=['conv_out']) * scale + offset
    def rewrite(self, op: ir.tape.Tape, x: ir.Value, w: ir.Value, b: ir.Value, scale: ir.Value, offset: ir.Value, conv_out: ir.Value) -> ir.Value:
        scale_value = scale.const_value.numpy()
        offset_value = offset.const_value.numpy()
        w_value = w.const_value.numpy()
