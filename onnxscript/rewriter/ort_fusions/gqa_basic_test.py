...
class GQA2(unittest.TestCase):
    ...
    def expanded_model_script(self):
        ...
        def gqa(query, key, value, past_key, past_value, cos, sin):
            ...
            # position_ids_q = op.Tile(position_ids, B) # op.Unsqueeze(position_ids, [0])
            # position_ids_k = op.Tile(position_ids, B) # op.Unsqueeze(position_ids, [0])
            ...
            one = op.Constant(value_int=1)
            past_plus_1 = op.Add(past_0D, one)
            mask = op.Trilu(all_min, past_plus_1, upper=1)
