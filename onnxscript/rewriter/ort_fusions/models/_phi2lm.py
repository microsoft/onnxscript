...
def make_model(
    ...
    @script()
    def main_graph(input_ids: INT64['s34','s16'], attention_mask: INT64['s34','s16 + s62'], past_key_values_key_cache_0: FLOAT['s34',32,'s62',80], past_key_values_value_cache_0: FLOAT['s34',32,'s62',80]) -> (FLOAT['s34','s16',51200], FLOAT['s34',32,'s16 + s62',80], FLOAT['s34',32,'s16 + s62',80]):
        ...
        full = opset18.Expand(-3.4028235e+38, val_15)
        ...
        masked_fill = opset18.Where(eq_64, -3.4028235e+38, slice_14)
        ...
        slice_scatter_2 = opset18.ScatterND(expand_1, val_210, slice_scatter_1, reduction='none')
        ...
        cos = opset18.Cos(cat)
        sin = opset18.Sin(cat)
        layer_norm = opset18.LayerNormalization(embedding, model_layers_0_input_layernorm_weight, model_layers_0_input_layernorm_bias, stash_type=1, epsilon=9.999999747378752e-06, axis=-1)
        ...
        layer_norm_1 = opset18.LayerNormalization(add_481, model_final_layernorm_weight, model_final_layernorm_bias, stash_type=1, epsilon=9.999999747378752e-06, axis=-1)
...
def make_model_with_random_weights():
    ...
    model_layers_0_self_attn_q_proj_weight = numpy.random.rand(2560,2560).astype(numpy.float32)
    model_layers_0_self_attn_q_proj_bias = numpy.random.rand(2560).astype(numpy.float32)
    model_layers_0_self_attn_k_proj_weight = numpy.random.rand(2560,2560).astype(numpy.float32)
    model_layers_0_self_attn_k_proj_bias = numpy.random.rand(2560).astype(numpy.float32)
    model_layers_0_self_attn_v_proj_weight = numpy.random.rand(2560,2560).astype(numpy.float32)
