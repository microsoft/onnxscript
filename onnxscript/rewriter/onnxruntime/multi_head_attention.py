...
def multi_head_attention_pattern (op, input, query_weight, key_weight, value_weight):
      query = project_transpose_head(op, input, query_weight)
      query_rope = op.Embed(query, _domain="local")
      key = project_transpose_head(op, input, key_weight)
      key_rope = op.Embed(key, _domain="local")
      ...
      attention = op.SDPA(query_rope, key_transposed, value, _allow_other_inputs=True, _domain="local")
...
def multi_head_attention(op, input, query_weight, key_weight, value_weight):
    ...
    return op.MultiHeadAttention(input, query_weight, key_weight, value_weight, _domain="local", _outputs=3)
