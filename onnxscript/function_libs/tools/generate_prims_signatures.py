...
def load_native_function_yaml(yaml_path: str):
    ...
    for func in all_functions:
        if "tags" not in func:
            continue
        # if isinstance(func["tags"], Sequence):
        #     continue;
...
def _get_func_schema_of_namespace(namespaces: List[_OpNamespace]) -> (Dict[str, FunctionSchema]):
    ...
    for op_namespace in namespaces:
        for attr_name in dir(op_namespace):
            ...
            print("=== op: ", attr_name)
            if attr_name in not_supported_ops or (not isinstance(op_overload_packet.return_type, tuple) and op_overload_packet.return_type.name.lower() != "new"):
                print("=== not supported op and schema: ", attr_name, getattr(op_overload_packet, 'schema'))
                continue

            func_schema = FunctionSchema.parse(getattr(op_overload_packet, 'schema'))
            table[attr_name] = func_schema

    return table

my_table = _get_func_schema_of_namespace([torch.ops.prims])
