...
def aten_floor_divide(self: TTensor, other: TTensor) -> TTensor:
    ...
    # The sign-mismatch check ``signbit(a) != signbit(b)`` is expressed as
    # ``(a < 0) == (b > 0)``, which avoids Sign (limited integral support on some
    # EPs) and lets ``(a < 0)`` short-circuit optimizations for non-negative ``a``.
    offset = op.And(
