...
def isomorphic (fn1 : onnx.FunctionProto, fn2: onnx.FunctionProto):
    ...
    if (fn1.output != fn2.output): return False
    ...
    def same_node(node1, node2):
        ...
        if (node1 in node_mapping):
            return node_mapping[node1] is node2 # Use reference equality
    ...
    for x in fn1.output:
        if not same_value(x, x): return False
