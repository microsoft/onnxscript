...
def istft(
    ...
    wone = op.Cast(
        op.ConstantOfShape(
            op.Shape(window), value=make_tensor("onef", TensorProto.FLOAT, [1], [1])
        ),
        to=1,
    )
    ...
    for fs in range(int(nf)):
        ...
        right = op.Cast(
            op.ConstantOfShape(
                right_shape, value=make_tensor("zerof", TensorProto.FLOAT, [1], [0])
            ),
            to=1,
        )
        left = op.Cast(
            op.ConstantOfShape(
                left_shape, value=make_tensor("zerof", TensorProto.FLOAT, [1], [0])
            ),
            to=1,
        )
