...
def do_name_resolution(fun: ast.FunctionDef, converter):
    ...
    def visit(stmt: ast.stmt, bindings):
        if isinstance(stmt, ast.Assign):
            ...
            resolve(stmt.test, bindings)
            then_scope = bindings.new_child()
            else_scope = bindings.new_child()
            visit_block(stmt.body, then_scope)
            visit_block(stmt.orelse, else_scope)
            # TODO: merge then_scope and else_scope
        elif isinstance(stmt, ast.For):
            update(bindings, stmt.target)
            loop_body_scope = bindings.new_child()
            visit_block(stmt.body, loop_body_scope)
            # TODO: merge loop_body_scope and bindings
            ...
            # a generalization.
            pass
        elif isinstance(stmt, ast.Expr) and hasattr(stmt, "value"):
            # docstring
            if hasattr(stmt.value, "value") and isinstance(stmt.value.value, str):
                # python 3.8+
                return
    ...
    assert isinstance(fun, ast.FunctionDef)
    bindings = ChainMap({})
    for x in fun.args.args:
        bindings[x.arg] = x
    visit_block(fun.body, bindings)