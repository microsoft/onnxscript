...
class _Adapter:
    def __init__(self, node_version: int, upgrade_version: int):
        self.original_version = node_version
        self.target_version = upgrade_version
    ...
    def process_node(self, node: ir.Node):
        ...
        op_adapters = registry.lookup_adapters(
            node.domain, node.op_type, self.original_version, self.target_version
        )
